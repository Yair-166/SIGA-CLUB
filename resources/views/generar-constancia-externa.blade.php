            ...
            <table style="border: 0px solid black; border-collapse: collapse; width: 100%;">
                <tr>
                    <td style='width: 18%; float: left'>
                        <img align="left" src="{{ URL::asset('images/' . $club->foto) }}" height="120" />
        ...
        <div id="cuerpo" style='width: 100%;'>
			...
			<p align="left">
                <span style="font-size:22px;font-family:Times;color:#000000;">
                    Participó en el <b>"{{$club->nombre}}”</b> en la actividad <b>{{$evento->nombre}}</b>
                </span>
                <br>
                <p align="left">
            ...
            <p align="left">
				<span class="ft17">
					...
					@if($evento->fechaInicio == $evento->fechaFin)
                        <b>{{$evento->fechaInicio}}</b>
                    @else
                        <b>{{$evento->fechaInicio}}</b> al <b>{{$evento->fechaFin}}</b>
                    @endif
                    </br>
                    Duración: <b>{{$asistencia->asistenciaTotal}} horas</b>.
                    </br>
                    Rol: <b>{{$rolactividad}}</b>.
                </span>
            </p>
        ...
        <div id="pie" style='width: 100%;'>
            <table>
                ...
                @foreach ($autoridades_externo as $autoridad)
                    @if($request->input('autoridad'.$autoridad->id) == $autoridad->id)
